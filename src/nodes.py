...
class NodeBase(QWidget):
    def __init__(self, session, summary:NodeSummary, title:str="", parent=None):
    ...
    def runCommand(self):
        ...
        if self.start_script_string != "":
            for command in self.start_script_string.splitlines():
                commands.run(self.session, command)
        if self.script_string != "":
            for command in self.script_string.splitlines():
                commands.run(self.session, command)
...
class NodePicker(NodeBase):
    def __init__(self, session, scene:Scene, summary:NodeSummary, all_views:bool, title:str="", selector_type:NodePickerType=NodePickerType.ModelPicker, parent=None):
...
class NodeColorPalette(NodeBase):
    def __init__(self, session, summary:NodeSummary, title:str="", colormap_height:int=50, parent=None):
        super().__init__(session, summary, title, parent)
...
class NodeTransparency(NodeBase):
    def __init__(self, session, summary:NodeSummary, title:str="", parent=None):
        super().__init__(session, summary, title, parent)
    ...
    def updateCommand(self) -> list[str]:
        ...
        if self.Run.isEnabled():
            ...
            if self.Height.currentText() == "Fixed":
                ...
                height = f"height {self.HeightValue.getText()}"
                self.HeightValue.setEnabled(True)
            self.script_string += f"label <i>'{objects}'</i> {label} {text_color} {bg_color} {height} size {self.Size.getText()} font \"{self.Font.currentText()}\" offset {self.OffsetX.getText()},{self.OffsetY.getText()},{self.OffsetZ.getText()} onTop {self.OnTop.isChecked()}<br>"
            self.script_string += f"<br>"
...
class NodeCenterRotation(NodeBase):
    def __init__(self, session, summary:NodeSummary, title:str="", parent=None):
        super().__init__(session, summary, title, parent)
    ...
    def updateCommand(self) -> list[str]:
        ...
        if self.Run.isEnabled():
            ...
            if self.Mark.isChecked():
                ...
                self.script_string += f"measure center <i>'{objects}'</i> mark True {radius} {color} {name}<br>"
            else:
...
class NodeDelete(NodeBase):
    def __init__(self, session, scene:Scene, summary:NodeSummary, title:str="", parent=None):
        super().__init__(session, summary, title, parent)
